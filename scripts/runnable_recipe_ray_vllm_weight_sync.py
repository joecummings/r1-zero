...
class RefActor:
    ...
    def _setup_model(self, cfg_model, ref_model_state_dict):
        ...
        log.info("done setting up ref model")
        return ref_model
    ...
    def run(self):
        ...
        while True:
            ...
            log.info(f"putting trajectory {trajectory} into actor queue")
...
class vLLMRolloutActor:
    def __init__(self, *args, queue, cfg, actor_id=-1, **kwargs):
        ...
        log.info(
            f"Actor CUDA_VISIBLE_DEVICES: {os.environ.get('CUDA_VISIBLE_DEVICES')}"
        )
    ...
    def rollout(self):
        ...
        for idx, batch in enumerate(self._dataloader):
            ...
            # log.info(self._tokenizer.decode(batch_tokens[0]))
            # log.info("===")
            # log.info(self._tokenizer.decode(postprocessed_results[0][0].cpu().numpy().tolist()))
            ...
            while True:
                try:
                    ...
                    log.info(f"rollout queue full. Discarding data.")
...
class vLLMWorkerWrapper(Worker):
    ...
    def __init__(self, *args, **kwargs):
        ...
        log.info(f"visible devices {os.environ['CUDA_VISIBLE_DEVICES']}")
        log.info(f"device count {torch.cuda.device_count()}")
...
class RayGRPORecipe:
    ...
    def start_ray(self):
        ...
        log.info("Cluster resources:", ray.cluster_resources())
